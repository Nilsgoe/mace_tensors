...
def test_run_train_dipole_cueq(tmp_path, fitting_configs):
    ...
    mace_params = _mace_params_dipole_cueq.copy()
    mace_params["checkpoints_dir"] = str(tmp_path)
    mace_params["model_dir"] = str(tmp_path)
    mace_params["train_file"] = tmp_path / "fit.xyz"
    ...
    calc_cueq = MACECalculator(model_paths=tmp_path / "DipolesMACE.model",model_type="DipoleMACE", device="cpu",cueq_enabled=True)
...
@pytest.mark.skipif(not CUET_AVAILABLE, reason="cuequivariance not installed")
def test_run_train_dipole_polar_cueq(tmp_path, fitting_configs):
    ase.io.write(tmp_path / "fit.xyz", fitting_configs)
    ...
    calc_cueq = MACECalculator(model_paths=tmp_path / "DielectricMACE.model",model_type="DipolePolarizabilityMACE", device="cpu",enable_cueq=True)
    ...
    print("Mus with CuEq", Mus_cueq)
    print("alphas with CuEq", alphas_cueq)
    # Obtained for MACE from the 08/08/2025 
    ref_Mus = [
        np.array([0., 0., 0.]),
        np.array([0., 0., 0.]),
