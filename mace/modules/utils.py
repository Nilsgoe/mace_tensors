...
def compute_hessian_autograd(
    ...
    if hessian_method == "vectorized_autograd":
            ...
            hessian= torch.autograd.grad(
                ...
                inputs = positions,#[positions]*len(grads[:-1]), 
                grad_outputs=tuple_of_tensors,#([torch.ones_like(forces.unsqueeze(1)) * torch.ones_like(forces)]*45),#tuple_of_tensors,#[torch.ones_like(grad_elem)]*len(grads),
                retain_graph=True,
                create_graph=False,
                allow_unused=False,
